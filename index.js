...
function processSum(numberList, callback) {
  const summm = numberList.reduce(function(accumulator, currentValue){
    return accumulator + currentValue
  },0);

  return callback(summm);
}
...
function firstNamesAllCaps(runners) {
 const allCaps = runners.map(function(drunk){
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const specificSize = runners.filter(function(findIt){
    return findIt.shirt_size == tShirtSize;
  });
  return specificSize;
}
...
function tallyUpDonations(runners) {
  const tallied = runners.reduce(function(myDonations, current){
    return myDonations + current.donation;
  },0);
  return tallied;
}
